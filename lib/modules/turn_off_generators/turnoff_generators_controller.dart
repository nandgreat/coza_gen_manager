...
import 'package:coza_gen_manager/models/turnoff_generator/turnoff_generator_response.dart';
import 'package:coza_gen_manager/models/turnoff_generator/turnoff_generator_response.dart';
...
class TurnoffGeneratorsController extends GetxController {
  ...
  Future<void> getPurposes() async {
    ...
    try {
      ...
      if (response.isOk) {
        purposes.value =
            purpose.GeneratorPurposeResponse.fromJson(response.body)
                .data!
                .toList();
        ...
        var message = purpose.GeneratorPurposeResponse.fromJson(response.body)
            .message
            .toString();
  ...
  Future<void> getAdmins() async {
    ...
    try {
      ...
      if (response.isOk) {
        admins.value =
            ad.ApprovalAdminResponse.fromJson(response.body).data!.toList();
        ...
        var message =
            ad.ApprovalAdminResponse.fromJson(response.body).message.toString();
  ...
  Future<void> turnOnGenerator() async {
    ...
    selectedGens.forEach((el) {
      var newElement =
          keys.firstWhere((element) => element.generatorName == el);
    ...
    try {
      Response response =
          await _generatorsRepository.turnOnGen(turnOnGenRequest);

      // ignore: unrelated_type_equality_checks
      if (response.isOk) {
        var message =
            TurnOnGenResponse.fromJson(response.body).message.toString();
  ...
  Future<void> turnOffGenerator() async {
    RxInt idFromFirstController = connectionManagerController.connectionType;
    ...
    var newElement = keys.firstWhere(
        (element) => element.generatorName == selectedActiveGen.value);

    logItem(selectedLoads.join(','));

    TurnoffGeneratorRequest turnOffGenRequest = TurnoffGeneratorRequest(
        dieselLevel: num.parse(dieselLevelController.text.isEmpty ? "-1" : dieselLevelController.text),
        generatorId: newElement.id,
        workerCode: workerCodeController.text);

    try {
      Response response =
          await _generatorsRepository.turnOffGen(turnOffGenRequest);

      // ignore: unrelated_type_equality_checks
      if (response.isOk) {
        var message =
            TurnoffGeneratorResponse.fromJson(response.body).message.toString();
